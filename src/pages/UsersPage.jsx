    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display font-bold text-2xl text-brand-900">Usuários</h1>
            <p className="font-mono text-xs text-ink-400 mt-1">gerenciamento de acesso</p>
          </div>
          <Button size="sm" onClick={() => setShowModal(true)}>+ Novo</Button>
        </div>
        ...
        <div className="bg-white border border-surface-border rounded-card shadow-card overflow-hidden">
          {loading ? (
            ...
            <div className="p-6 text-center font-mono text-xs text-ink-400">Nenhum usuário encontrado</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {users.map((u, i) => (
                  <tr key={u.id} className={i % 2 === 0 ? 'bg-white' : 'bg-brand-50'}>
                    <td className="px-4 py-3">{u.name || '—'}</td>
                    <td className="px-4 py-3 font-mono text-xs">{u.email}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-block px-2 py-0.5 rounded-full font-mono text-[10px] font-semibold
                        ${u.role === 'admin' ? 'bg-amber-50 text-amber-600' : 'bg-brand-100 text-brand-900'}`}>
                        {u.role}
                      </span>
                    </td>
                  </tr>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Novo Usuário">
        <form onSubmit={handleCreate} className="space-y-3">
          <Input label="Nome"  placeholder="Nome completo"   {...field('name')} />
          <Input label="E-mail" type="email" placeholder="email@exemplo.com" required {...field('email')} />
          <Input label="Senha"  type="password" placeholder="••••••••" required {...field('password')} />
          <div>
            <label className="block text-xs font-bold text-ink-900 mb-1">Perfil</label>
            <select
              {...field('role')}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button type="submit" loading={saving} fullWidth>Criar</Button>
          </div>
        </form>
      </Modal>
    </AppLayout>
