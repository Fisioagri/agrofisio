  ...
  return (
    ...
    >
      <h3 className="font-display font-bold text-base text-brand-900 mb-5">
        {initial ? '✏️ Editar Talhão' : '➕ Novo Talhão'}
      </h3>
      <form onSubmit={e => { e.preventDefault(); onSave(form) }} className="space-y-4">
        ...
        <div>
          <label className={label}>CAR (Cadastro Ambiental Rural)</label>
...
export default function TalhoesPage() {
  ...
  useEffect(() => {
    Promise.all([listTalhoes(), listProdutores()])
      .then(([t, p]) => { setTalhoes(t); setProdutores(p) })
      .catch(() => setError('Erro ao carregar talhões.'))
      .finally(() => setLoading(false))
  }, [])

  const filtered = talhoes.filter(t => {
    const matchSearch = t.nome.toLowerCase().includes(search.toLowerCase()) ||
      (t.cultura || '').toLowerCase().includes(search.toLowerCase())
    const matchProd = !filterProd || t.produtor_id === filterProd
    return matchSearch && matchProd
  })

  const totalHa = filtered.reduce((s, t) => s + (parseFloat(t.area_ha) || 0), 0)
  ...
  async function handleDelete(id) {
    if (!window.confirm('Excluir este talhão?')) return
    setDeleting(id)
    try {
    ...
    <AppLayout>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="font-display font-bold text-2xl text-brand-900">🗺️ Talhões</h1>
            <p className="font-mono text-xs text-ink-400 mt-1">
              {talhoes.length} talhão{talhoes.length !== 1 ? 's' : ''} · {totalHa.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} ha total
            </p>
          </div>
          {!showForm && (
            <button
            ...
            {!showForm && (
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  placeholder="🔍 Buscar talhão ou cultura…"
                  value={search}
            ...
            {loading ? (
              ...
              <div className="bg-white border border-surface-border rounded-card p-8 text-center">
                ...
                <p className="font-mono text-xs text-ink-400">
                  {search ? 'Nenhum resultado.' : 'Nenhum talhão cadastrado ainda.'}
                </p>
                {!search && (
                  ...
                  {filtered.map(t => (
                    <motion.div
                      ...
                      className={`bg-white border rounded-card p-4 shadow-card flex items-center gap-3 transition-colors
                        ${panel?.id === t.id ? 'border-brand-700' : 'border-surface-border hover:border-brand-400'}`}
                    >
                      <div className="w-10 h-10 rounded-lg bg-brand-100 flex items-center justify-center text-xl flex-shrink-0">
                        {CROP_ICO[t.cultura] || '🌱'}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-display font-bold text-sm text-brand-900 truncate">{t.nome}</p>
                        <p className="font-mono text-[10px] text-ink-400 mt-0.5">
                          {[
                            t.cultura && t.cultura.charAt(0).toUpperCase() + t.cultura.slice(1),
                            t.area_ha && `${parseFloat(t.area_ha).toLocaleString('pt-BR')} ha`,
                            t.safra,
                          ].filter(Boolean).join(' · ')}
                        </p>
                        {t.produtores?.nome && (
                          <p className="font-mono text-[10px] text-ink-300 mt-0.5">👤 {t.produtores.nome}</p>
                        )}
                      </div>
                      <div className="flex gap-1.5 flex-shrink-0">
                        <button
                          onClick={() => setPanel(panel?.id === t.id ? null : t)}
