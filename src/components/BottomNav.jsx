...
function shareReport(data, manipHtml, shareTitle) {
  const text = manipHtml
    ? manipHtml.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
    : 'Protocolo gerado pelo AgroFísio'
  ...
  if (navigator.share) {
    ...
    navigator.clipboard.writeText(msg)
      .then(() => alert('Protocolo copiado! Cole no WhatsApp ou onde preferir.'))
  ...
  if (step === 7) {
    return (
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-surface-border p-3 flex gap-2.5 max-w-2xl mx-auto z-50">
        ...
        <button
          onClick={() => shareReport(data, manipHtml, 'AgroFísio Report')}
