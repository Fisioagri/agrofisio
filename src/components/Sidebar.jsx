...
function Icon({ path, path2, size = 18 }) {
  return (
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[220px] flex-shrink-0 h-screen sticky top-0 bg-white border-r border-surface-border flex flex-col">
      {/* Logo */}
      <div className="bg-brand-900 px-4 py-5 flex-shrink-0">
      ...
      <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
        {NAV_MAIN.map(item => {
          ...
          return (
            <motion.div key={item.to} whileHover={{ x: active ? 0 : 3 }} transition={{ duration: 0.12 }}>
              <Link
                to={item.to}
                className={[
                  'flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[11px] font-mono transition-colors',
                  active
                    ? 'bg-brand-50 text-brand-900 font-medium border-l-[3px] border-brand-900 pl-[9px]'
                    : 'text-ink-400 hover:bg-surface-bg hover:text-ink-600',
                ].join(' ')}
              >
      ...
      {/* Bottom */}
      <div className="border-t border-surface-border p-2 space-y-0.5 flex-shrink-0">
        <Link
          to="/settings"
          className={[
            'flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-mono transition-colors',
            pathname === '/settings'
        ...
        <div className="flex items-center gap-1.5 px-3 py-2">
          <span className="font-mono text-[10px] text-ink-300 flex-1">Idioma</span>
          <button
            onClick={() => setLang('pt')}
        ...
        <button
          onClick={signOut}
          className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-mono text-ink-400 hover:bg-danger-50 hover:text-danger-600 transition-colors"
        >
          <Icon path="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" size={16} />
          Sair
        </button>
      </div>
    </aside>
